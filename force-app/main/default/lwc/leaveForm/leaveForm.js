...
this.leaveDescription = this.value;
    ...
    handleChangeDays(event) {
       ...
       this.intialIndex = parseInt(event.target.dataset.index);
        ...
        //this.totalDayCount = this.intialDayCount;
...
HandleSumbit(){
   ...
   insertLeavRecord({
       CandidateId : this.recordId,LeaveMasterDetailsId : this.leaveMasterDetailID,LeaveDaysCount : this.totalDayCount,ApplyStartDate:this.startdate,ApplyEndDate:this.enddate,LeaveDescription:this.leaveDescription,leavePlanObj:this.dateList
   })
   .then(result=>{
       if(result){
           this.showSuccessMessage();
           var BaseURL =  window.location.href.slice(0,63);
          //  BaseURL'lightning/r/Leave_Transaction__c/'+result.Id+'view'; 
